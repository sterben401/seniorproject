...
<body>
    <div class="container">
        ...
        <a type="button" class="btn btn-danger" href="admin.php" >Back</a>
    </div>

    <script>
        var todayButton = document.getElementById("today");
        var threeDaysButton = document.getElementById("3days");
        var sevenDaysButton = document.getElementById("7days");
        var oneMonthButton = document.getElementById("1month");
        var deleteAllRecordsButton = document.getElementById("deleteAllRecords");
        ...
        function createTable(data) {
            ...
            data.forEach(item => {
                ...
                row.innerHTML = `<td>${item.Date_detec}</td><td>${item.Patterns}</td><td>${item.Count}</td><td><span style="color: ${item.Status}">${item.Status}</span></td>`;
